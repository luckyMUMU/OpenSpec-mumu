...
| **停止点** | 等待`[WAITING_FOR_TEST_REVIEW]` | 绕过停止点 | 测试未审核 |
...
| **架构文档(.pseudo)** | Prometheus（读写） | 其他角色 | 非Prometheus修改架构 |
| **需求文档** | Analyst（读写） | 其他角色 | 非Analyst修改需求 |
| **实现设计(design.md)** | Oracle（读写） | 其他角色 | 非Oracle修改设计 |
...
| `src/**/design.md` | Oracle | 其他角色 | 非Oracle修改设计 |
