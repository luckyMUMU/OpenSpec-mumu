...
## 上下文
- 项目背景: 已有用户注册功能，需要添加登录
- 已知约束: 使用Rust语言，Web框架是Axum
```

**对话过程**:
```
Analyst: 您希望解决什么问题？
User: 用户需要登录后才能访问某些功能

Analyst: 登录方式有哪些？用户名密码？还是支持第三方？
User: 先支持用户名密码，后续可能加GitHub登录

Analyst: 登录后需要保持会话多久？
User: 7天免登录

Analyst: 安全方面有什么要求？比如密码加密、登录失败限制？
User: 密码要加密，连续5次失败要锁定15分钟
```
...
### PRD 文档
- **位置**: `docs/01_requirements/auth_login_prd.md`
...
| 业务 | 支持用户登录，7天免登录 | 低 |
| 用户 | 已注册用户 | 低 |
| 功能 | 用户名密码登录、会话管理、失败锁定 | 中 |
| 技术 | JWT + Redis会话，bcrypt加密 | 中 |
| 风险 | 安全漏洞、会话劫持 | 中 |
| 验收 | 登录成功/失败场景、锁定机制 | 低 |
