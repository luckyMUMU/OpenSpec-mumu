...
> **主要变更**: 新增 P0-SEC-004/005、更新 Skill 调用规范、改进需求澄清流程
...
### 6.1.1 P0 约束检查清单（12 条）

**SEC 安全类（5 条）**:
- [ ] P0-SEC-001: 无硬编码密钥（git-secrets 扫描）
- [ ] P0-SEC-002: 无已知漏洞库（npm audit/Snyk 扫描）
- [ ] P0-SEC-003: 安全校验未关闭（代码审查）
- [ ] P0-SEC-004: 网络访问白名单配置（网络策略审计）
- [ ] P0-SEC-005: 密钥安全注入（密钥管理系统审计）

**QUAL 质量类（3 条）**:
- [ ] P0-QUAL-001: 核心模块覆盖率 100%（istanbul/coverage.py）
- [ ] P0-QUAL-002: 无强制解包（ESLint/clippy）
- [ ] P0-QUAL-003: 无忽略错误（静态分析）

**ARCH 架构类（2 条）**:
- [ ] P0-ARCH-001: 无循环依赖（dependency-cruiser）
- [ ] P0-ARCH-002: 无跨层调用（架构审查）

**COMP 合规类（2 条）**:
- [ ] P0-COMP-001: 数据隐私保护（GDPR 检查）
- [ ] P0-COMP-002: 许可证合规（license-checker）
