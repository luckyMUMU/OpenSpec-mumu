...
步骤2: 全局更新（主/次版本变更时）
  ├─ 更新所有文档的版本号
  ├─ 更新CHANGELOG.md
  └─ 更新Skill文件版本号（主/次版本必须与SOP一致；修订可独立递增）
...
- [ ] SKILL.md 正文仅包含“做事骨架”（触发条件/步骤顺序/输出格式/质量门槛），长内容必须下沉
- [ ] 长知识已下沉到 references/，且 SKILL.md 以链接引用（按需加载）
- [ ] 重复动作已沉淀为 scripts/，且 SKILL.md 通过“调用脚本”复用

#### 3.6.1 SKILL.md 只写“做事骨架”

- SKILL.md 必须只写可执行的“做事骨架”，禁止承载长背景/长解释/长示例/长FAQ（这些内容必须下沉到 references/ 并以链接引用）
- SKILL.md 必须显式写清并可被审查验证：
  - **触发条件**：仅当用户输入命中关键词/信号时 → 才能启用该 Skill；否则禁止启用
  - **步骤顺序**：Step 1/2/3…（命令式：仅当/必须/禁止/否则），不得跳步
  - **输出格式**：必须声明固定小标题与必含字段（用于落盘与下游复用）
  - **质量门槛**：必须定义“完成”判定；当输入不足/存在冲突/依赖缺口时必须标注“待确认”，并触发对应停止点
- SKILL.md 允许包含 YAML frontmatter（至少 `name`、`description`；如由生成器生成可包含 `metadata.*` 等字段），但正文仍必须遵循“骨架”原则

#### 3.6.2 把“长知识”放进 references/

- 仅当内容“每次都要看且直接影响执行顺序”时 → 才能保留在 SKILL.md；否则必须移动到 references/ 并以链接引用
- references/ 必须用于按需加载的长内容：规范文档、示例输出、模板、术语表、FAQ、长篇最佳实践
- 目录落点必须明确且可追溯（择一或组合）：
  - SOP 通用标准/模板 → `docs/参考/sop/04_reference/`
  - 项目运行期资料（RAG） → `docs/04_context_reference/rag/**/references/`（参见 `sop/04_reference/knowledge_management.md`）
  - Skill 私有参考 → `docs/参考/sop/skills/<skill-name>/references/`

#### 3.6.3 重复动作做成 scripts/

- 当某操作满足“每次都差不多且需要稳定复用”（如套模板、格式化报表、批量改文件名、生成标准化输出）时 → 必须优先沉淀为脚本；禁止在对话中反复生成“差不多的脚本”
- 参考案例：将反复出现的品牌模板套用逻辑固化为 `apply_template.py`，后续直接调用脚本复用
- scripts/ 落点规则必须清晰：
  - Skill 私有脚本 → `docs/参考/sop/skills/<skill-name>/scripts/`
  - 跨 Skill/仓库维护脚本 → 仓库根 `scripts/`（并在 `scripts/README.md` 登记）
- SKILL.md 的步骤必须通过“调用脚本”表达复用（写清输入/输出与落盘路径），禁止粘贴脚本全文到 SKILL.md
...
- [ ] SKILL.md 正文仅包含“做事骨架”，并声明固定输出小标题与字段
- [ ] 长知识不写入 SKILL.md，必须下沉至 references/ 并被引用
- [ ] 重复动作不写入 SKILL.md，必须以 scripts/ 固化并被调用
